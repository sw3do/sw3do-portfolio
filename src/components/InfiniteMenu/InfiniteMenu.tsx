...
class InfiniteGridMenu {
  ...
  private initTexture(): void {
    ...
    Promise.all(
      this.items.map(
        (item, index) =>
          new Promise<{ img: HTMLImageElement; index: number } | null>(
            (resolve) => {
              const img = new Image();
              img.crossOrigin = "anonymous";
              img.onload = () => {
                console.log(`Image ${index} loaded successfully:`, item.title);
                resolve({ img, index });
              };
              img.onerror = () => {
                console.warn(
                  `Failed to load image ${index} (${item.title}), skipping:`,
                  item.image,
                );
                resolve(null);
              };
      ...
      const validResults = results.filter(
        (r): r is { img: HTMLImageElement; index: number } => r !== null,
      );
      console.log(
        `Successfully loaded ${validResults.length}/${results.length} images`,
      );
...
const InfiniteMenuComponent: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  }, [items, handleActiveItem, handleMovementChange]);
  ...
  return (
    <div className="relative w-full h-full">
      <canvas
        ...
        ref={canvasRef}
        className="w-full h-full overflow-hidden relative outline-none cursor-grab active:cursor-grabbing touch-none"
        aria-label="3D Interactive Skills Menu"
      />
        ...
        <>
          ...
          {activeItem.link && (
            <button
              ...
              aria-label={`Visit ${activeItem.title}`}
              className={`
                absolute
                left-1/2
                -translate-x-1/2
                ...
                ${
                  isMoving
                    ? "-bottom-20 opacity-0 pointer-events-none scale-0"
                    : "bottom-[8%] opacity-100 pointer-events-auto scale-100"
                }
              `}
            >
              <span
                className="select-none relative text-black translate-y-0.5 text-[20px] sm:text-[26px]"
                aria-hidden="true"
              >
                &#x2197;
              </span>
...
const InfiniteMenu = memo(InfiniteMenuComponent);
